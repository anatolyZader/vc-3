...
class FineGrainedCodeSplitter {
  constructor(options = {}) {
    ...
    // Use existing TokenBasedSplitter for accurate token counting
  ...
  countTokens(text) {
    return this.tokenSplitter.countTokens(text);
  }
  ...
  async splitDocument(document) {
    ...
    try {
      ...
      for (const unit of semanticUnits) {
        ...
        // Special handling for Fastify registrations - keep them together even if large
        if (tokenCount > this.maxTokens && !this.isFastifyRegistration(unit)) {
  ...
  extractAllSemanticUnits(ast, lines, metadata) {
    ...
    traverse(ast, {
      ...
      // Extract every class method
      ClassMethod: (path) => {
        const unit = this.createSemanticUnit(path.node, lines, 'class_method', 'method');
        if (unit) {
  ...
  isImportantCall(node) {
    ...
    if (node.callee.type === 'MemberExpression') {
      ...
      // Fastify registrations and routes - these should be kept as complete units
  ...
  isFastifyRegistration(unit) {
    // Check if this unit is a Fastify registration that should be kept together
    return unit.type === 'call_expression' && 
           (unit.callType === 'plugin_registration' || 
            unit.callType === 'route_definition' ||
            unit.content.includes('fastify.register('));
  }
...
async function testCustomFineGrainedSplitting() {
  ...
  try {
    // Read app.js from current directory
    const appJsPath = path.join(__dirname, 'app.js');
    const originalCode = fs.readFileSync(appJsPath, 'utf8');
    console.log(`📄 Original app.js: ${originalCode.length} characters\n`);

    // Create custom splitter with larger token limit for semantic coherence
    const splitter = new FineGrainedCodeSplitter({
      maxTokens: 500,      // Increased to keep complete registrations together
      minTokens: 30,
      overlapTokens: 50
    });
