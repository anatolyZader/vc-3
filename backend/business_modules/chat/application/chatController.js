...
// Update the addQuestion method in your existing chatController.js
fastify.decorate('addQuestion', async (request, reply) => {
  ...
  fastify.decorate('addQuestion', async (request, reply) => {
    try {
      ...
      setTimeout(async () => {
        try {
          const aiResponse = `Hello anatolyZader! I received your message: "${prompt}". This is a simulated AI response. In production, this would be generated by your AI service.`;
          await chatService.addAnswer(userId, conversationId, aiResponse);
          fastify.log.info(`AI response added to conversation ${conversationId}`);
        } catch (aiError) {
          fastify.log.error('Error adding AI response:', aiError);
