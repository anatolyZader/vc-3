...
async function chatController(fastify, options) {
  ...
  fastify.decorate('addQuestion', async (request, reply) => {
    try {
      ...
      setImmediate(async () => {
        try {
          const aiResponse = `Hello ${request.user.name || 'there'}! I received your message: "${prompt}". This is a simulated AI response that would be generated by your AI service in staging.`;
