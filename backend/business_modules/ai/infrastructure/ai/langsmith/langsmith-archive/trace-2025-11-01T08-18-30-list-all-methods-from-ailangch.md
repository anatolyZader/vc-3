...
<!--
═══════════════════════════════════════════════════════════════════════════════
LANGSMITH RAG TRACE ANALYSIS - DOCUMENTATION
═══════════════════════════════════════════════════════════════════════════════

PURPOSE:
This file provides comprehensive analysis of RAG (Retrieval-Augmented Generation)
query execution, capturing every step from vector search to AI response generation.
It helps debug, optimize, and understand how the RAG pipeline retrieves and uses
...
1. 🔍 QUERY DETAILS
   ...
   - User and conversation identifiers for tracking
   - Execution timestamps and duration
   - Used to understand what was asked and how long it took

2. 🔗 LANGSMITH TRACE INFORMATION
   - LangSmith observability platform metadata
   - Trace IDs for correlating with LangSmith UI
   - Environment and project configuration
   - Links this trace to external monitoring tools

3. PIPELINE EXECUTION STEPS
   - Sequential breakdown of RAG pipeline stages
   - Each step shows: name, timestamp, status, and metrics
   - Helps identify bottlenecks and failures in the pipeline
   - Steps include: initialization, vector search, text search, context building, response

4. 📊 VECTOR SEARCH ANALYSIS
   - Configuration used for vector similarity search
   - Documents retrieved and total context size
   - Source type distribution (code, docs, specs)
   - Helps understand what types of content were retrieved

5. 🏷️ UBIQUITOUS LANGUAGE (UL) TAGS ANALYSIS
   - Shows how many chunks have domain terminology tagged
   - Lists bounded contexts, business modules, and domain terms
   - Coverage percentage indicates if repository needs re-indexing
   - Helps verify domain-driven design metadata is properly applied

6. 📋 COMPLETE CHUNK ANALYSIS
   - Detailed breakdown of EVERY chunk retrieved
   - Shows: source file, content, metadata, UL tags, similarity score
   - Full content allows manual verification of retrieval quality
   - Metadata inspection reveals indexing and enhancement status

7. 🤖 AI RESPONSE ANALYSIS
   - The final generated response from the LLM
   - Quality assessment (relevance, context usage, completeness)
   - Shows how well the AI used the retrieved context
   - Response length and key elements extracted

8. 📈 PERFORMANCE METRICS
   - Query processing time breakdown
   - Context quality scores (relevance, diversity, completeness)
   - Search efficiency metrics
   - LangSmith integration status

9. 🔍 SOURCE ANALYSIS
   - Most frequently used source files
   - Repository coverage statistics
   - Helps understand which files are most relevant

10. 🎯 QUERY CLASSIFICATION & ANALYSIS
    - Automatic categorization of query type
    - Domain focus detection
    - Technical complexity assessment
    - Helps optimize search strategies for different query types

11. 🚀 RECOMMENDATIONS
    - Actionable suggestions for improving RAG performance
    - Identifies missing documentation or context gaps
    - Chunking optimization suggestions
    - Re-indexing recommendations

12. ✨ CONCLUSION
    - Overall performance summary
    - Quality grades for retrieval and response
    - Success/failure determination
    - Quick glance assessment

HOW TO USE THIS FILE:
- Check "UL Tags Analysis" first to verify domain metadata coverage
- Review "Complete Chunk Analysis" to see what was retrieved
- Look at "Pipeline Execution Steps" to identify slow stages
- Read "Recommendations" for actionable improvements
- Compare "Query Details" duration with performance benchmarks

TROUBLESHOOTING:
- Missing UL tags? Repository may need re-indexing (check processedAt timestamps)
- Low relevance scores? Query may need refinement or more context
- Long duration? Check which pipeline step is slowest
- Wrong chunks retrieved? Review search strategy and filters
- Empty results? Vector store may not have relevant content indexed

GENERATED BY: QueryPipeline.generateComprehensiveTraceAnalysis()
FILE LOCATION: backend/business_modules/ai/infrastructure/ai/langsmith/latest-trace-analysis.md
AUTO-ARCHIVED TO: langsmith-archive/trace-{timestamp}-{query-slug}.md
...
## 🔍 Query Details
<!-- 
  This section captures the original user query and execution context.
  - Query: The exact prompt/question the user asked
  - User ID: Unique identifier for the user (used for personalized retrieval)
  - Conversation ID: Groups related queries in a conversation thread
  - Timestamps: When execution started and completed
  - Duration: Total time from query to response (target: <10 seconds)
-->
...
<!--
  ...
  This section links this trace to the LangSmith UI for deeper analysis.
  - Project: LangSmith project name (configured in LANGCHAIN_PROJECT env var)
  - Tracing Enabled: Whether LangSmith tracing is active
  - Trace ID: Unique identifier for this trace in LangSmith (view in UI)
  - Run ID: Specific run identifier within the trace
  - Environment: development, staging, or production
  
  🔗 View in LangSmith UI: https://smith.langchain.com/
-->
...
<!--
  RAG Pipeline stages in chronological order:
  1. initialization - Setup vector store and dependencies
  2. vector_store_check - Verify vector database is available
  3. vector_search - Semantic similarity search in Pinecone
  4. text_search - Keyword-based search in PostgreSQL (BM25)
  5. hybrid_search_combination - Merge vector + text results
  6. context_building - Format chunks for LLM consumption
  7. response_generation - LLM generates answer using context
  
  ⚠️ If any step shows 'failed' status, that's where debugging should focus.
  ⏱️ Most time typically spent in: vector_search and response_generation
-->
...
<!--
  Configuration and results of semantic vector search.
  - Vector Store: Which vector database (primary = Pinecone)
  - Search Strategy: Algorithm used to select chunks
    * intelligent_strategy_with_filters: Adaptive search with content-type filters
    * repository_specific_search: Search within specific repo namespace
  - Documents Retrieved: Number of chunks returned (typical: 10-60)
  - Total Context: Combined character count (LLM context window limit: ~200K tokens)
  
  💡 More chunks ≠ better results. Quality over quantity!
-->
...
<!--
  Breakdown of retrieved content by type:
  - GitHub Repository Code: Actual source code files (.js, .ts, .py, etc.)
  - Module Documentation: Business module README/docs (business_modules/**/MODULE_NAME.md)
  - Architecture Documentation: Root-level docs (ARCHITECTURE.md, ROOT_DOCUMENTATION.md)
  - API Specification: OpenAPI/Swagger specs (httpApiSpec.json)
  - Other Sources: Configs, tests, or unclassified content
  
  🎯 Ideal mix depends on query type:
     - Code questions: 70-80% code, 20-30% docs
     - Architectural questions: 50% docs, 50% code
     - API questions: 60% API spec, 40% code
-->
...
<!--
  ═══════════════════════════════════════════════════════════════════════════
  DETAILED CHUNK-BY-CHUNK BREAKDOWN
  ═══════════════════════════════════════════════════════════════════════════
  
  This section shows EVERY chunk retrieved from the vector database.
  Each chunk represents a piece of code or documentation that was deemed
  relevant to the user's query based on semantic similarity.
  
  CHUNK METADATA FIELDS EXPLAINED:
  
  📁 Source: The file path or identifier
     - Format: owner/repo or file path
     - Used to locate the original file
  
  🏷️ Type: Content classification
     - github-code: Source code files
     - github-docs: Documentation/markdown files
     - github-test: Test files
     - github-config: Configuration files
     - module_documentation: Module-level docs
     - architecture_documentation: Root architecture docs
  
  📏 Size: Character count of the chunk
     - Typical range: 500-3000 characters
     - Larger chunks may be truncated by LLM context limits
  
  ⭐ Score: Similarity score (0.0 - 1.0)
     - How semantically similar to the query
     - Scores > 0.6: Highly relevant
     - Scores 0.4-0.6: Moderately relevant  
     - Scores < 0.4: Marginally relevant
     - Score 1.0: Explicitly requested file (file-specific query)
  
  🏢 Repository: Git repository identifier
     - Format: owner/repo-name
     - Used for multi-repo knowledge bases
  
  🌿 Branch: Git branch name
     - Usually 'main' or 'master'
     - Allows versioned documentation
  
  📂 File Type: Programming language or file type
     - js, ts, py, md, json, yaml, etc.
     - Used for syntax highlighting and AST parsing
  
  ⏰ Processed At: When the file was last indexed
     - ISO 8601 timestamp
     - Old timestamps may indicate stale content
     - If > 7 days old, consider re-indexing
  
  🏷️ UBIQUITOUS LANGUAGE (UL) TAGS:
  
  Domain-Driven Design metadata added during indexing:
  
  - ul_version: Version of UL tagging schema (e.g., "ul-1.0.0")
  - ul_bounded_context: DDD bounded context (e.g., "ai", "chat", "git")
  - ul_terms: Array of domain terms found (e.g., ["Repository", "Aggregate", "Entity"])
  - ul_match_count: Number of UL terms detected
  - ubiq_business_module: Business module name (e.g., "auth", "docs", "api")
  - ubiq_bounded_context: Legacy bounded context field
  - ubiq_domain_events: Domain events related to this code
  - ubiq_terminology: Business terminology detected
  - ubiq_enhanced: Whether UL enhancement was applied
  - ubiq_enhancement_timestamp: When UL tagging occurred
  
  ✅ Present = Chunk has domain metadata for better retrieval
  ❌ Missing = Chunk lacks domain metadata (may need re-indexing)
  
  WHY UL TAGS MATTER:
  - Improve semantic search accuracy
  - Enable domain-aware filtering
  - Help LLM understand business context
  - Support DDD (Domain-Driven Design) practices
  - Allow bounded context-specific queries
...
<!--
  CHUNK 1 DETAILS:
  This is the first (and usually most relevant) chunk retrieved.
  Chunks are ordered by similarity score (highest first).
-->
...
<!--
  ...
  The actual text/code that was retrieved and sent to the LLM.
  
  WHAT TO CHECK:
  ✅ Relevance: Does this content relate to the user's query?
  ✅ Completeness: Is the code/text complete or truncated?
  ✅ Quality: Is this helpful information or noise?
  ✅ Accuracy: Is the code current or outdated?
  
  COMMON ISSUES:
  ❌ Truncated functions/classes (check if chunk boundaries are clean)
  ❌ Wrong file retrieved (score may be misleading)
  ❌ Outdated code (check processedAt timestamp)
  ❌ Analysis markdown files instead of source code
  
  💡 TIP: If many chunks are irrelevant, the query may need refinement
         or the vector embeddings need re-generation.
-->
```
